...
describe('runCleanupJob', () => {
  it('deletes old transactions and logs count', async () => {
    ...
    expect(console.log).toHaveBeenCalledWith(expect.stringContaining('Deleted 3'));
  });
...
describe('runReportJob', () => {
  it('logs no transactions when result is empty', async () => {
    ...
    await runReportJob();
    expect(console.log).toHaveBeenCalledWith(expect.stringContaining('No transactions found'));
  });

  it('logs each row when transactions exist', async () => {
    mockQuery.mockResolvedValueOnce({
      rows: [{ type: 'deposit', status: 'completed', count: 5, total_amount: '1000' }],
    });
    await runReportJob();
    expect(console.log).toHaveBeenCalledWith(expect.stringContaining('deposit'));
  });
});
...
describe('runStatusCheckJob', () => {
  it('logs no stuck transactions when result is empty', async () => {
    ...
    expect(console.log).toHaveBeenCalledWith('[status-check] No stuck transactions found');
  });
  ...
  it('warns for each stuck transaction', async () => {
    ...
    await runStatusCheckJob();
    expect(console.warn).toHaveBeenCalledWith(expect.stringContaining('1 stuck'));
  ...
  it('uses STUCK_TRANSACTION_MINUTES env var', async () => {
    ...
    mockQuery.mockResolvedValueOnce({ rows: [] });
    await runStatusCheckJob();
    expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining('30 minutes'));
